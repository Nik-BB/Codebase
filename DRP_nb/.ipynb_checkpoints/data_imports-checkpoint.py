...
def load_ka(path, re_fmt_indx=True):
    '''Function to load kinease atalas matrix with light pre-processing

    '''
    raw_ka = pd.read_csv(path)
    #remove empty cols
    empty = raw_ka.columns[raw_ka.isna().sum() == len(raw_ka)]
    ka = raw_ka.drop(columns=empty)

    if re_fmt_indx:
        #reformat index to be the same as phos omics data
        mask = ka['Gene'].notna()
        ka_site = ka[mask]
        ser_left = pd.Series('(', index=ka_site.index)
        ser_right = pd.Series(')', index=ka_site.index)
        end = pd.Series(';', index=ka_site.index)
        index = ka_site['Gene'] + ser_left + ka_site['Phosphosite'] + ser_right + end
        ka_site.index = index
        #check no duplicates 
        assert (ka_site.duplicated() == False).all()
